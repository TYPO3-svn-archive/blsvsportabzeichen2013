...

{
	...
	{
		$this->ErgebnisArray =& $daten['ergebnisse'];
		$tmp_file = $opt['datei_tmp'];
		$bitmap = $opt['bild'];		
...
// $this->write_tabelle($tabelle);
	...
	function create_($daten, &$opt = array())
	{
		$content = '';
		$info =& $daten['info'];
		$tabelle =& $daten['tabelle'];
	...
	function erstellePage($bitmap, $agArray)
	{
		...
		$this->worksheet->setColumn(1, 1, 3, $this->format[locked]);
